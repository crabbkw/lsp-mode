...
(defun lsp--get-ignored-regexes-for-workspace-root (workspace-root)
  ...
  (let* (
         (temp-buffer-path (expand-file-name ".lsp-mode-temp-file"
                                             (expand-file-name workspace-root)))
         (temp-buffer (find-file-noselect temp-buffer-path 't))
         (ignored-files (buffer-local-value 'lsp-file-watch-ignored-files temp-buffer))
         (ignored-directories (buffer-local-value 'lsp-file-watch-ignored-directories temp-buffer))
         )
    (kill-buffer temp-buffer)
    (list ignored-files ignored-directories)))
